...
import { InterceptorFile, MultipartFile } from '../../../multipart/interfaces/multipart-file.interface';
...
describe('MultipartWrapper', () => {
	...
	beforeEach(() => {
		...
		fileObject.fields[objectFieldname] = [
			fileObject
		];
		...
		describe('options', () => {
			describe('dest', () => {
				...
				it('should call writeFile() with expected params if dest is defined', async () => {
					...
					expect(writeFileSpy.calledWith(fileObject.fields[objectFieldname][0])).to.be.true;
				});
			});
			describe('fileFilter', () => {
		...
		describe('options', () => {
			...
			describe('fileFilter', () => {
				...
				it('should throw error if options.fileFilter callback is (Error, Boolean)', async () => {
					...
					const multipart = new MultipartWrapper(options);
					return expect(multipart.files(arrayFieldname)(req)).to.be.rejected.and.to.eventually.equal(newHttpError);
				});
			});
		});
	});
		...
		describe('options', () => {
			...
			describe('fileFilter', () => {
				...
				it('should throw error if options.fileFilter callback is (Error, Boolean)', async () => {
					...
					const multipart = new MultipartWrapper(options);
					return expect(multipart.any()(req)).to.be.rejected.and.to.eventually.equal(newHttpError);
				});
			});
		});
	});
	describe('fileFields', () => {
		...
		describe('options', () => {
			...
			describe('fileFilter', () => {
				...
				it('should omit specific file if callback is (null, false)', async () => {
					const fileToOmitInArray: InterceptorFile = filesArray[1];
					const fileToOmit: InterceptorFile = fileObject;
					const options: MultipartOptions = {
						fileFilter: (req, file, cb) => {
				...
				it('should throw error if options.fileFilter callback is (Error, Boolean)', async () => {
					...
					return expect(multipart.fileFields([
						{ name: arrayFieldname, maxCount: 10 },
						{ name: objectFieldname, maxCount: 10 },
					])(req)).to.be.rejected.and.to.eventually.equal(newHttpError);
				});
			});
		});
	});
